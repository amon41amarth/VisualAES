...
import pygame, slowaes, sys, time
...
def gui():
    ...
    clearData = []
    for x in range(200):
            clearData.append(cleartext)

    ciphData = []
    for x in range(200):
            ciphData.append(ciph)
    ...
    while True:
        ...
        if ballrect.top < 0 or ballrect.bottom > height:
            speed[1] = -speed[1]
        clearImage = Image.new('RGB', (200,200))
        clearImage.putdata(clearData)
        ciphImage = Image.new('RGB', (200,200))
        ciphImage.putdata(ciphData)
        screen.fill(black)
        screen.blit(ball, ballrect)
        screen.blit(clearImage, (0,0))
        screen.blit(ciphImage, (200,0))
        pygame.display.flip()
        time.sleep(.01)
